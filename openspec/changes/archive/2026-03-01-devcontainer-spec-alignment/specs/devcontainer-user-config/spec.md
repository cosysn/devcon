...
### Requirement: Container environment variables (spec-compliant)
The devcontainer configuration SHALL support specifying container environment variables through the spec-compliant `containerEnv` property.

#### Scenario: Container env specified
- **WHEN** devcontainer.json specifies containerEnv
- **THEN** the environment variables SHALL be accessible in the container
